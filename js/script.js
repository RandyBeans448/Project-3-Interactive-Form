...
//Getting a refrence to the activities div and creating a div to append a new sting to hold the total cost of the clicked activities.
const activities = document.querySelector('.activities');
let totalActivityCost = document.createElement('div');
activities.appendChild(totalActivityCost);
let totalCost = 0;
//Setting the text content to tell the total and adding the total cost which will calculted later.
...
activities.addEventListener('change', e => {
  const clicked = e.target;
        if (clicked.checked) {
            totalCost +=
            parseInt(clicked.getAttribute('data-cost'));
          } else {
            totalCost -=
            parseInt(clicked.getAttribute('data-cost'));
...
//These function test to see the entries in the input boxes are vaild when compared to the regex. The color
...
let inputCard = document.getElementById('cc-num');
  const cardVaildator = () => {
    const cardValue = inputCard.value;
      let cardRegex = /\d{4}\-\d{4}\-\d{4}\-\d{4}/i;
...
//The event listener runs the vaildation functions on the inputs submissions.
